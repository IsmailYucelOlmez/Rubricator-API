-- Include source column in semantic search RPC results (advanced ingest books).
-- Return type changed (added `source`), so the existing function must be dropped first.
